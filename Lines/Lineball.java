...
public class Lineball {
  ...
  public void StartGame() {
    ...
    for (i = 0; i < MaxCell; i++) for (j = 0; j < MaxCell; j++) balltmp[i][j] =
      ball[i][j];
    // Sinh random 3 mau se~ xuat hien
    new3color();
    for (int k = 0; k < 3; k++) nextcolortmp[k] = nextcolor[k];
  }

  //-------------------------------------------------------------------
  public void new3color() { //tao 3 mau sap xuat hien
    Random random = new Random();
    for (int k = 0; k < 3; k++) nextcolor[k] = random.nextInt(MaxColor) + 1;
  }
  ...
  public void Undo() {
    ...
    for (int k = 0; k < 3; k++) nextcolor[k] = nextcolortmp[k];
  ...
  public void saveUndo() {
    for (int i = 0; i < MaxCell; i++) for (int j = 0; j < MaxCell; j++) if (
      ball[i][j] > 2 * MaxColor
    ) balltmp[i][j] = ball[i][j] - 2 * MaxColor; else balltmp[i][j] =
      ball[i][j];
    for (int k = 0; k < 3; k++) nextcolortmp[k] = nextcolor[k];
  ...
  public boolean cutBall() {
    ...
    for (i = 0; i < MaxCell; i++) for (
      j = 0;
      j < MaxCell;
      j++
    ) CheckBall[i][j] = false;

    for (nRow = 0; nRow < MaxCell; nRow++) for (
      nCol = 0;
      nCol < MaxCell;
      nCol++
      ...
      if (nCount >= 5) {
        for (i = 0; i < nCount; i++) CellBall[NumCutBall++] = TempBall[i];
    ...
    for (i = 0; i < NumCutBall; i++) ball[CellBall[i].x][CellBall[i].y] = 0;
    if (NumCutBall > 0) return true; else return false;
  }
  ...
  public void FindPath(point p, point[][] PathBallTemp) {
    if (p.x != -1 && p.y != -1) if (
      PathBallTemp[p.x][p.y] != new point(-1, -1)
  ...
  public boolean Loang(int si, int sj, int fi, int fj) { // Loang de tim duong di tu (si,sj)-->(fi,fj);
    ...
    for (i = 0; i < MaxCell; i++) for (j = 0; j < MaxCell; j++) if (
      ball[i][j] > 0 && ball[i][j] < 8
    ) ballCheck[i][j] = true; else ballCheck[i][j] = false;

    ballCheck[pStart.x][pStart.y] = true;
    if (ballCheck[fi][fj]) return false;
    ...
    while (nQuery > 0) {
      ...
      for (int nLast = 0; nLast < nQuery; nLast++) {
        ...
        for (k = 0; k < 4; k++) {
          ...
          j = pCurrent.y + dj[k];
          if (
            i >= 0 && i < MaxCell && j >= 0 & j < MaxCell && !ballCheck[i][j]
          ) {
      ...
      for (k = 0; k < nCount; k++) Query[0][k] = Query[1][k];
      nQuery = nCount;
    }
