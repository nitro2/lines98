...
public class LineFrame extends JFrame {
  ...
  public LineFrame() {
    ...
    for (int i = 0; i < 9; i++) for (int j = 0; j < 9; j++) add(button[i][j]);
  ...
  public void moveBall(int i, int j, int ii, int jj) throws Exception {
    ...
    a.ball[i][j] = 0;
    for (int k = 0; k < 22; k++) if (
      button[i][j].getIcon() == icon[k]
  ...
  public void stopGame() throws IOException {
    if (a.gameover == true) {
      ...
      for (int i = 0; i < 10; i++) if (
        topScores.player[i].scores < player.scores
      ) {
        k2 = true;
        break;
      }
